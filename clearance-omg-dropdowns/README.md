...
This produces:
- `references/field-groups.json` – one record per **interactive PDF field name**.
  - **Radio groups** include `options[]` with `exportValue`, `onState`, and `displayLabel`.
    - If `/Opt` is numeric (e.g., `0..9`), `displayLabel` is inferred from nearby page text.
  - **Dropdowns (ComboBox/ListBox)** include `options[]` extracted from the field’s `/Opt` list (the real selectable values).
    - `rawOptions` preserves the exact PDF order (including rare duplicates) for strict validation.
...
Your Next.js UI can now render **all radio and dropdown options** deterministically without hard-coding them.

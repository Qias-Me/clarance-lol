...
def main() -> int:
    ...
    # Build dropdown groups (ComboBox / ListBox)
    dropdown_widgets = [w for w in enhanced_widgets if w.get("type") in ("ComboBox", "ListBox")]
    for w in dropdown_widgets:
        fname = w["name"]
        raw_opts = w.get("options") or []

        # Normalize to display strings (use second element if [export, label] pair)
        opt_strings: List[str] = []
        for o in raw_opts:
            if isinstance(o, (list, tuple)) and len(o) >= 2:
                opt_strings.append(str(o[1]))
            elif isinstance(o, str):
                opt_strings.append(o)

        # De-dupe for UI (keep rawOptions for strict validation)
        dedup: List[str] = []
        seen: Set[str] = set()
        ...
        display_label = w.get("displayLabel") or w.get("label") or w.get("inferredLabel") or fname
        ...
        field_groups[fname] = {
            ...
            "sectionPart": w.get("sectionPart"),
            "subsection": w.get("subsection"),
            "entry": w.get("entry"),
            "pageIndex": int(w.get("pageIndex", -1)),
            "options": [
                {"label": s, "exportValue": s, "uiLabel": s, "displayLabel": s}
                for s in dedup
            ],
            "rawOptions": opt_strings,
            "widgets": [
                {"stableId": w.get("stableId"), "rectTopLeft": w.get("rectTopLeft")}
            ],
        }
